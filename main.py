...
is_break = True
stage = False
admins = [116399612, 73985833, 152900351, 185274846, 348430773, 324822588, 63294601, 154248724]
...
def kickMember(user):
    ...
    global users
    if len(current_queue) > 0:
        print("Кикнуть", user, users[current_queue[0]], user in users[current_queue[0]])
            ...
            if event.from_user:
                ...
                # ---- Добавить смайлик ----
                    ...
                    if event.user_id in admins:
                        ...
                        if text[:4] == "!кик":
                            kickMember(event.text[5:])
                        if text[:5] == "!kick":
                            kickMember(event.text[6:])
